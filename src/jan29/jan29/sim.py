...
class FollowCarAfterTrigger(Node):
    ...
    def __init__(self):
        ...
        ################################################
        # [GAZEBO SIM FIX] use_sim_time 강제 (Nav2/TF 시간 불일치 방지)
        ################################################
        ...
        ################################################
        # [GAZEBO SIM FIX] 카메라 토픽을 Gazebo 시뮬용 기본 토픽으로 교체
        # - 실제 사용하는 launch에 따라 아래 토픽들이 다를 수 있음
        # - 터틀봇4_gz / turtlebot4_ignition_bringup 환경에서 흔한 예시로 구성
        ################################################
        ...
        self.yolo_weights = "/home/jeonguk/rokey_project_E2_AI_ros/best.pt"
        ...
        ################################################
        # [GAZEBO SIM FIX] depth stamp 저장해서 TF 변환 시 "현재시간(Time())" 대신 사용
        # - 시뮬/분산 환경에서 Time() 사용하면 TF cache mismatch/Message Filter drop 자주 발생
        ################################################
        ...
        ################################################
        # [GAZEBO SIM FIX] 시뮬에선 use_sim_time 켜진 상태에서 Nav2 활성화 대기
        ################################################
        ...
        #########################################
        ################################################
        # [GAZEBO SIM FIX] 이미지/카메라 정보는 센서 QoS 스타일(BEST_EFFORT + depth=1)이 안정적
        ################################################
        cam_qos = QoSProfile(
        ...
        ################################################
        # [GAZEBO SIM FIX] 구독 QoS를 cam_qos로 통일 (특히 이미지/카메라정보)
        ################################################
        self.create_subscription(CameraInfo, self.info_topic, self.on_info, cam_qos)
        self.create_subscription(Image, self.rgb_topic, self.on_rgb, cam_qos)
        self.create_subscription(Image, self.depth_topic, self.on_depth, cam_qos)
        ...
        try:
            ...
            if depth is not None and depth.size > 0:
                with self.lock:
                    ...
                    ################################################
                    # [GAZEBO SIM FIX] depth 메시지 stamp 저장
                    ################################################
    ...
    def pixel_to_map_point_depthtomap_style(self, u: int, v: int):
        with self.lock:
            ...
            ################################################
            # [GAZEBO SIM FIX] 변환에 사용할 stamp 확보
            ################################################
            stamp = self.last_depth_stamp

        if depth is None or K is None or frame_id is None or stamp is None:
            return None
        ...
        ################################################
        # [GAZEBO SIM FIX] Time().to_msg() 대신 "depth stamp" 사용 (TF cache mismatch 방지)
        ################################################
    ...
    def tick(self):
        ...
        if self.state == self.SEARCH_CAR:
            with self.lock:
                ...
                stamp_ok = (self.last_depth_stamp is not None)

            # ✅ 로그 폭탄 제거: 상태만 출력
            if rgb is None or (not K_ok) or (not depth_ok) or depth_frame is None or (not stamp_ok):
                self.get_logger().warn(
                    f"[WAIT] rgb={'OK' if rgb is not None else 'None'} "
                    f"K={'OK' if K_ok else 'None'} "
                    f"depth={'OK' if depth_ok else 'None'} "
                    f"stamp={'OK' if stamp_ok else 'None'} "
            ...
            ################################################
            # [GAZEBO SIM FIX] base_link도 "현재시간(Time())" 대신 노이즈 적은 clock 사용
            # - 여기서는 ROS clock(now) 사용(시뮬 시간 기반)
            ################################################
            base_pt.header.stamp = self.get_clock().now().to_msg()
